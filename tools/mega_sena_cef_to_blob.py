...
def _resolve_field_map(headers: List[str]) -> _FieldMap:
    ...
    # Bolas/dezenas: aceitamos "Bola1..Bola6" e variações com underscore/espaço.
    ...
    if len(bola_norm_to_original) < 6:
        # Segundo padrão comum: "bola_01", "bola_02" etc ou "bola 01"
        for norm, orig in by_norm.items():
            m = re.fullmatch(r"(?:bola|dezena)(?:_)?0?(\d{1,2})", norm)
            if m:
                idx = int(m.group(1))
                if 1 <= idx <= 6 and (idx, orig) not in bola_norm_to_original:
                    bola_norm_to_original.append((idx, orig))

    bola_norm_to_original = sorted({idx: orig for idx, orig in bola_norm_to_original}.items())
    bola_keys = [orig for _, orig in bola_norm_to_original if 1 <= _ <= 6]

    if len(bola_keys) != 6:
        raise ValueError(
            "não foi possível localizar 6 colunas de dezenas (Bola1..Bola6). "
...
def _iter_rows(path: str) -> Iterable[Dict[str, str]]:
    with _open_text_best_effort(path) as f:
        ...
        # - Se não, assumir layout posicional: concurso, data, bola1..bola6, (opcional winners6)
        ...
        header_markers = {"concurso", "numero_concurso", "data_sorteio", "bola1", "bola_1", "dezena1", "winners_6"}
        ...
        # Layout esperado (mínimo): 1 + 1 + 6 = 8 colunas.
        # Layout com winners6: 9 colunas.
        values = first
        col_count = len(values)
        if col_count < 8:
            raise ValueError(
                "arquivo sem header e com poucas colunas. "
                f"esperado >= 8 (concurso, data, 6 dezenas). obtido {col_count}."
            )

        headers = ["Concurso", "Data Sorteio"] + [f"Bola{i}" for i in range(1, 7)]
        if col_count >= 9:
            headers.append("Ganhadores 6 acertos")
...
def convert_cef_csv_to_blob_document(path: str) -> Dict[str, List[dict]]:
    ...
    for i, row in enumerate(rows, start=2):  # header é a linha 1
        ...
        if len(numbers) != 6:
            raise ValueError(f"linha {i}: esperado 6 dezenas, obtido {len(numbers)}")

        winners_6 = 0
        if field_map.winners6_key:
            winners_6 = _parse_int(row[field_map.winners6_key], field=field_map.winners6_key, row_index=i)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--input", "-i", required=True, help="Caminho para o CSV da CEF (histórico Mega-Sena).")
